...
class DetectionPanel(BasePanel):
    ...
    def CtrlLine(self):
        ...
        if self.CheckBoxEnhance.GetValue() == True:
            self.useEnhance = True
            self.CtrlResult, result1, result2 \
            = algorithm3_with_slope(im, line_parameters, True, self.useFilter, True, False)
        else:
            self.useEnhance = False
            self.CtrlResult, result1, result2 \
            = algorithm3_with_slope(im, line_parameters, False, self.useFilter, True, False)
        ...
        min_value = np.around(self.CtrlResult[5], decimals=3)
        max_different = np.around(self.CtrlResult[2], decimals=3)

        self.CtrlDFArr = self.CtrlResult[6]
    ...
    def TestLine(self, event):
        ...
        result, df_array, max_different, max_index, max_value, min_index, min_value = find_peak(im, parameters, self.useFilter, True, False)

        self.TestMinIndex = min_index
        self.TestMinValue = min_value
        self.TestMaxIndex = max_index
        self.TestMaxValue = max_value
        self.TestMaxDiff = max_different

        self.TestDFArr = df_array
