...
def get_line_para_map(disease_chosen):
    LINE_STRIDE = 20
    line_para_map = {'FluA':[[2, 40, 240, LINE_STRIDE, 50],[1, 300, 500, LINE_STRIDE, 9.372]], 
                     'FluB':[[2, 350, 550, LINE_STRIDE, 50],[1, 80, 280, LINE_STRIDE, 5.767]], 
                     'Myco':[[2, 650, 850, LINE_STRIDE, 50],[1, 80, 280, LINE_STRIDE, 5.783]],
                     'RSV-hMPV':[[2, 600, 800, LINE_STRIDE, 40],[1, 50, 250, LINE_STRIDE, 10.211],[2, 330, 530, LINE_STRIDE, 7.261]],
                     'StrepA':[[2, 650, 850, LINE_STRIDE, 40],[1, 80, 280, LINE_STRIDE, 5.486]]}
...
def gamma_enhance(im, enhance_value=2):
    ...
    #im = cv2.bilateralFilter(im, 5, 21, 21)
...
def white_balance(im):
    ...
    r = np.asarray(r * kr, np.uint8)
    g = np.asarray(g * kg, np.uint8)
    b = np.asarray(b * kb, np.uint8)

    im = cv2.merge([r, g, b])
    
    return im

def find_peak(im, parameters, useWhiteBalance, useFliter, smoothing_value, usePeakDetection, show):
    ...
    if useFliter:        
        ...
        df_array = median_filter(df_array, smoothing_value)
    ...
    return result, df_array, max_different, taget_max_point[0], taget_max_point[1], taget_min_point[0], taget_min_point[1]  

def calculateCutOffValue(all_neg_files, line_parameters, useGammaEnhance, useWhiteBalance, useSmoothingFilter, smoothing_value, std_rate=4):
    neg_list1 = []
    neg_list2 = []
    for neg_i in all_neg_files:
        im_n = cv2.imread(neg_i, cv2.IMREAD_COLOR)
        im_n = cv2.cvtColor(im_n, cv2.COLOR_BGR2RGB)
        control, test_result1, test_result2 = algorithm3_with_slope(im_n, line_parameters, useGammaEnhance, useWhiteBalance, useSmoothingFilter, smoothing_value, True, False)
        if control[0]==1:
            print(neg_i)
...
def calculateCutOffValue4Block(all_neg_files, line_parameters, useGammaEnhance, useWhiteBalance, useSmoothingFilter, smoothing_value, std_rate=4):
    neg_list1 = []
    neg_list2 = []
    for neg_i in all_neg_files:
        im_n = cv2.imread(neg_i, cv2.IMREAD_COLOR)
        im_n = cv2.cvtColor(im_n, cv2.COLOR_BGR2RGB)
        control, test_result1, test_result2, _, _ = algorithm3_with_slope_4block(im_n, line_parameters, useGammaEnhance, useWhiteBalance, useSmoothingFilter, smoothing_value, True, False)
        if control[0]==1:
...
def algorithm3_with_slope(im, line_parameters, useGammaEnhance, useWhiteBalance, useFliter, smoothing_value, usePeakDetection, show_figure):
    ...
    c_result, c_df_array, c_max_different, max_index, max_value, min_index, min_value = find_peak(im, control_line_parameters, useWhiteBalance, useFliter, smoothing_value,  usePeakDetection, False)
    ...
    result1, df_array1, max_different1, max_index1, max_value1, min_index1, min_value1 = find_peak(im, test_line1_parameters, useWhiteBalance, useFliter, smoothing_value, usePeakDetection, show_figure)
    ...
    if len(line_parameters)==3:
        ...
        result2, df_array2, max_different2, max_index2, max_value2, min_index2, min_value2 = find_peak(im, test_line2_parameters, useWhiteBalance, useFliter, smoothing_value, usePeakDetection, show_figure)
...
def algorithm3_with_slope_4block(im, line_parameters, useGammaEnhance, useWhiteBalance, useFliter, smoothing_value, usePeakDetection, show_figure, num_positive=4):
    ...
    for i in range(4):
        ...
        #detect test line1
        test_line1_parameters = line_parameters[1]
        result1, df_array1, max_different1, max_index1, max_value1, min_index1, min_value1 = find_peak(block_im, test_line1_parameters, useWhiteBalance, useFliter, smoothing_value, usePeakDetection, show_figure)
        ...
        if len(line_parameters)==3:
            test_line2_parameters = line_parameters[2]
            result2, df_array2, max_different2, max_index2, max_value2, min_index2, min_value2 = find_peak(block_im, test_line2_parameters, useWhiteBalance, useFliter, smoothing_value, usePeakDetection, show_figure)
